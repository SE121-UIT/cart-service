...
import { MongoClient, Collection, Document, ObjectId, UpdateResult } from 'mongodb';
...
export const getMongoCollection = async <Doc extends Document>(
  ...
  const { databaseName, collectionName } =
    typeof options !== 'string' ? options : { databaseName: undefined, collectionName: options };
...
export const assertUpdated = async (update: () => Promise<UpdateResult>): Promise<UpdateResult> => {
...
export const assertFound = async <T>(find: () => Promise<T | null>): Promise<T> => {
...
export const getCheckpointsCollection = () => getMongoCollection<Checkpoint>('checkpoints');
