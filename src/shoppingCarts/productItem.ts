...
export const addProductItem = (
  ...
  return productItems.map((pi) => (pi.productId === productId ? mergedProductItem : pi));
};
...
export const removeProductItem = (
  ...
  const currentProductItem = assertProductItemExists(productItems, newProductItem);

  const newQuantity = currentProductItem.quantity - quantity;

  if (newQuantity === 0) return productItems.filter((pi) => pi.productId !== productId);
