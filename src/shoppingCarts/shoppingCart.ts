...
export const isCashierShoppingCartEvent = (event: unknown): event is ShoppingCartEvent => {
  return (
    event != null &&
    ((event as ShoppingCartEvent).type === 'shopping-cart-opened' ||
      (event as ShoppingCartEvent).type === 'product-item-added-to-shopping-cart' ||
      (event as ShoppingCartEvent).type === 'product-item-removed-from-shopping-cart' ||
